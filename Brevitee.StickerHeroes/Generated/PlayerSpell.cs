...
namespace Brevitee.StickerHeroes
{
	...
	public Spell SpellOfSpellId
	{
		...
		{
			return new PlayerSpellCollection(new Query<PlayerSpellColumns, PlayerSpell>(where, db), true);
		}
		...
		{
			return new PlayerSpellCollection(new Query<PlayerSpellColumns, PlayerSpell>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerSpellCollection(Select<PlayerSpellColumns>.From<PlayerSpell>().Where(where, db));
		}
		...
		{
			var results = new PlayerSpellCollection(Select<PlayerSpellColumns>.From<PlayerSpell>().Where(where, db));
		...
		public static PlayerSpellCollection Top(int count, WhereDelegate<PlayerSpellColumns> where, OrderBy<PlayerSpellColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerSpell>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerSpellCollection>(0);
		}
