...
namespace Brevitee.StickerHeroes
{
	...
	public Weapon WeaponOfWeaponId
	{
		...
		{
			return new PlayerOneWeaponCollection(new Query<PlayerOneWeaponColumns, PlayerOneWeapon>(where, db), true);
		}
		...
		{
			return new PlayerOneWeaponCollection(new Query<PlayerOneWeaponColumns, PlayerOneWeapon>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerOneWeaponCollection(Select<PlayerOneWeaponColumns>.From<PlayerOneWeapon>().Where(where, db));
		}
		...
		{
			var results = new PlayerOneWeaponCollection(Select<PlayerOneWeaponColumns>.From<PlayerOneWeapon>().Where(where, db));
		...
		public static PlayerOneWeaponCollection Top(int count, WhereDelegate<PlayerOneWeaponColumns> where, OrderBy<PlayerOneWeaponColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerOneWeapon>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerOneWeaponCollection>(0);
		}
