...
namespace Brevitee.StickerHeroes
{
	...
	public Weapon WeaponOfWeaponId
	{
		get
		{
			...
			{
				_weaponOfWeaponId = Brevitee.StickerHeroes.Weapon.OneWhere(f => f.Id == this.WeaponId);
			}
		...
		{
			return new RequiredLevelWeaponCollection(new Query<RequiredLevelWeaponColumns, RequiredLevelWeapon>(where, db), true);
		}
		...
		{
			return new RequiredLevelWeaponCollection(new Query<RequiredLevelWeaponColumns, RequiredLevelWeapon>(where, orderBy, db), true);
		}
		...
		{
			return new RequiredLevelWeaponCollection(Select<RequiredLevelWeaponColumns>.From<RequiredLevelWeapon>().Where(where, db));
		}
		...
		{
			var results = new RequiredLevelWeaponCollection(Select<RequiredLevelWeaponColumns>.From<RequiredLevelWeapon>().Where(where, db));
		...
		public static RequiredLevelWeaponCollection Top(int count, WhereDelegate<RequiredLevelWeaponColumns> where, OrderBy<RequiredLevelWeaponColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<RequiredLevelWeapon>(): database;
			...
			query.Execute(db);
			return query.Results.As<RequiredLevelWeaponCollection>(0);
		}
