...
namespace Brevitee.StickerHeroes
{
	...
	[Brevitee.Data.Column(Name="Level", ExtractedType="", MaxLength="", AllowNull=false)]
	public int? Level
	{
		get
		{
	...
	[Brevitee.Data.Column(Name="ExperiencePoints", ExtractedType="", MaxLength="", AllowNull=false)]
	...
	public PlayerEquipmentCollection PlayerEquipmentsByPlayerId
	{
		...
		{
			return new PlayerCollection(new Query<PlayerColumns, Player>(where, db), true);
		}
		...
		{
			return new PlayerCollection(new Query<PlayerColumns, Player>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerCollection(Select<PlayerColumns>.From<Player>().Where(where, db));
		}
		...
		{
			var results = new PlayerCollection(Select<PlayerColumns>.From<Player>().Where(where, db));
		...
		public static PlayerCollection Top(int count, WhereDelegate<PlayerColumns> where, OrderBy<PlayerColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<Player>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerCollection>(0);
		}
