...
namespace Brevitee.BattleStickers
{
	...
	public Spell SpellOfSpellId
	{
		...
		{
			return new PlayerOneSpellCollection(new Query<PlayerOneSpellColumns, PlayerOneSpell>(where, db), true);
		}
		...
		{
			return new PlayerOneSpellCollection(new Query<PlayerOneSpellColumns, PlayerOneSpell>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerOneSpellCollection(Select<PlayerOneSpellColumns>.From<PlayerOneSpell>().Where(where, db));
		}
		...
		{
			var results = new PlayerOneSpellCollection(Select<PlayerOneSpellColumns>.From<PlayerOneSpell>().Where(where, db));
		...
		public static PlayerOneSpellCollection Top(int count, WhereDelegate<PlayerOneSpellColumns> where, OrderBy<PlayerOneSpellColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerOneSpell>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerOneSpellCollection>(0);
		}
