...
namespace Brevitee.BattleStickers
{
	...
	public Spell SpellOfSpellId
	{
		get
		{
			...
			{
				_spellOfSpellId = Brevitee.BattleStickers.Spell.OneWhere(f => f.Id == this.SpellId);
			}
		...
		{
			return new RequiredLevelSpellCollection(new Query<RequiredLevelSpellColumns, RequiredLevelSpell>(where, db), true);
		}
		...
		{
			return new RequiredLevelSpellCollection(new Query<RequiredLevelSpellColumns, RequiredLevelSpell>(where, orderBy, db), true);
		}
		...
		{
			return new RequiredLevelSpellCollection(Select<RequiredLevelSpellColumns>.From<RequiredLevelSpell>().Where(where, db));
		}
		...
		{
			var results = new RequiredLevelSpellCollection(Select<RequiredLevelSpellColumns>.From<RequiredLevelSpell>().Where(where, db));
		...
		public static RequiredLevelSpellCollection Top(int count, WhereDelegate<RequiredLevelSpellColumns> where, OrderBy<RequiredLevelSpellColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<RequiredLevelSpell>(): database;
			...
			query.Execute(db);
			return query.Results.As<RequiredLevelSpellCollection>(0);
		}
