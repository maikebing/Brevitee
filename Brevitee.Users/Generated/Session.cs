...
namespace Brevitee.Users.Data
{
	...
	[Brevitee.Data.Table("Session", "Users")]
	...
	[Brevitee.Data.Column(Name="Identifier", ExtractedType="", MaxLength="", AllowNull=false)]
	...
	[Brevitee.Data.Column(Name="CreationDate", ExtractedType="", MaxLength="", AllowNull=true)]
	...
	[Brevitee.Data.Column(Name="LastActivity", ExtractedType="", MaxLength="", AllowNull=true)]
	...
	[Brevitee.Data.Column(Name="IsActive", ExtractedType="", MaxLength="", AllowNull=true)]
	...
	public UserBehaviorCollection UserBehaviorsBySessionId
	{
		...
		{
			return new SessionCollection(new Query<SessionColumns, Session>(where, db), true);
		}
		...
		{
			return new SessionCollection(new Query<SessionColumns, Session>(where, orderBy, db), true);
		}
		...
		{
			return new SessionCollection(Select<SessionColumns>.From<Session>().Where(where, db));
		}
		...
		{
			var results = new SessionCollection(Select<SessionColumns>.From<Session>().Where(where, db));
		...
		public static SessionCollection Top(int count, WhereDelegate<SessionColumns> where, OrderBy<SessionColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<Session>(): database;
			...
			query.Execute(db);
			return query.Results.As<SessionCollection>(0);
		}
