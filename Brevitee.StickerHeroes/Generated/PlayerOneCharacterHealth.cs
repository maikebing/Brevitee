...
namespace Brevitee.StickerHeroes
{
	...
	public Character CharacterOfCharacterId
	{
		...
		{
			return new PlayerOneCharacterHealthCollection(new Query<PlayerOneCharacterHealthColumns, PlayerOneCharacterHealth>(where, db), true);
		}
		...
		{
			return new PlayerOneCharacterHealthCollection(new Query<PlayerOneCharacterHealthColumns, PlayerOneCharacterHealth>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerOneCharacterHealthCollection(Select<PlayerOneCharacterHealthColumns>.From<PlayerOneCharacterHealth>().Where(where, db));
		}
		...
		{
			var results = new PlayerOneCharacterHealthCollection(Select<PlayerOneCharacterHealthColumns>.From<PlayerOneCharacterHealth>().Where(where, db));
		...
		public static PlayerOneCharacterHealthCollection Top(int count, WhereDelegate<PlayerOneCharacterHealthColumns> where, OrderBy<PlayerOneCharacterHealthColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerOneCharacterHealth>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerOneCharacterHealthCollection>(0);
		}
