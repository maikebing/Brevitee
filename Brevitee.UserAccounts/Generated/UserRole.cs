...
namespace Brevitee.UserAccounts.Data
{
	...
	public Role RoleOfRoleId
	{
		get
		{
			...
			{
				_roleOfRoleId = Brevitee.UserAccounts.Data.Role.OneWhere(f => f.Id == this.RoleId);
			}
		...
		public static UserRoleCollection LoadAll(Database database = null)
		{
			...
			sql.Select<UserRole>();
			Database db = database == null ? Db.For<UserRole>(): database;
		...
		{
			var results = new UserRoleCollection(db, new Query<UserRoleColumns, UserRole>(where, db), true);
			return results;
		}
		...
		{
			var results = new UserRoleCollection(db, new Query<UserRoleColumns, UserRole>(where, orderBy, db), true);
			return results;
		}
		...
		{
			var results = new UserRoleCollection(db, Select<UserRoleColumns>.From<UserRole>().Where(where, db));
			return results;
		}
