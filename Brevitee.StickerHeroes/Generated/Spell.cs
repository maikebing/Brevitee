...
namespace Brevitee.StickerHeroes
{
	...
	public RequiredLevelSpellCollection RequiredLevelSpellsBySpellId
	{
		...
		{
			return new SpellCollection(new Query<SpellColumns, Spell>(where, db), true);
		}
		...
		{
			return new SpellCollection(new Query<SpellColumns, Spell>(where, orderBy, db), true);
		}
		...
		{
			return new SpellCollection(Select<SpellColumns>.From<Spell>().Where(where, db));
		}
		...
		{
			var results = new SpellCollection(Select<SpellColumns>.From<Spell>().Where(where, db));
		...
		public static SpellCollection Top(int count, WhereDelegate<SpellColumns> where, OrderBy<SpellColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<Spell>(): database;
			...
			query.Execute(db);
			return query.Results.As<SpellCollection>(0);
		}
