...
namespace Brevitee.Razor
{
    public class RazorParser<TBaseTemplate> where TBaseTemplate: RazorBaseTemplate
    {
        ...
        {
            this.ResultInspector = (s) => { Console.WriteLine(s); };
        ...
        {
            Assembly[] reference = new Assembly[]{ typeof(TBaseTemplate).Assembly, typeof(Providers).Assembly};
            return Execute(input, reference, options, inspector);
        ...
        /// <returns></returns>
        public string Execute(TextReader input,  Assembly[] assembliesToReference, object options = null, Action<string> inspector = null)
        {
            CSharpCodeProvider codeProvider = new CSharpCodeProvider();
            GeneratorResults results = _engine.GenerateCode(input);

            string[] assemblyNames = assembliesToReference.Select(a => a.CodeBase.Replace("file:///", "").Replace("/", "\\")).ToArray();
            CompilerResults compilerResults = codeProvider.CompileAssemblyFromDom(
                new CompilerParameters(assemblyNames),
