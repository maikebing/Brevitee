...
namespace Brevitee.BattleStickers.Business.Data
{
	...
	public RequiredLevelSkillCollection RequiredLevelSkillsBySkillId
	{
		...
		public static SkillCollection LoadAll(Database database = null)
		{
			...
			sql.Select<Skill>();
			Database db = database == null ? Db.For<Skill>(): database;
		...
		{
			var results = new SkillCollection(db, new Query<SkillColumns, Skill>(where, db), true);
			return results;
		}
		...
		{
			var results = new SkillCollection(db, new Query<SkillColumns, Skill>(where, orderBy, db), true);
			return results;
		}
		...
		{
			var results = new SkillCollection(db, Select<SkillColumns>.From<Skill>().Where(where, db));
			return results;
		}
