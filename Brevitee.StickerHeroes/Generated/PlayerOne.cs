...
namespace Brevitee.StickerHeroes
{
	...
	public Battle BattleOfBattleId
	{
		get
		{
			...
			{
				_battleOfBattleId = Brevitee.StickerHeroes.Battle.OneWhere(f => f.Id == this.BattleId);
			}
	...
	public PlayerOneEquipmentCollection PlayerOneEquipmentsByPlayerOneId
	{
		...
		{
			return new PlayerOneCollection(new Query<PlayerOneColumns, PlayerOne>(where, db), true);
		}
		...
		{
			return new PlayerOneCollection(new Query<PlayerOneColumns, PlayerOne>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerOneCollection(Select<PlayerOneColumns>.From<PlayerOne>().Where(where, db));
		}
		...
		{
			var results = new PlayerOneCollection(Select<PlayerOneColumns>.From<PlayerOne>().Where(where, db));
		...
		public static PlayerOneCollection Top(int count, WhereDelegate<PlayerOneColumns> where, OrderBy<PlayerOneColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerOne>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerOneCollection>(0);
		}
