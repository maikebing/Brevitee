...
namespace Brevitee.StickerHeroes
{
	...
	public Character CharacterOfCharacterId
	{
		...
		{
			return new PlayerCharacterCollection(new Query<PlayerCharacterColumns, PlayerCharacter>(where, db), true);
		}
		...
		{
			return new PlayerCharacterCollection(new Query<PlayerCharacterColumns, PlayerCharacter>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerCharacterCollection(Select<PlayerCharacterColumns>.From<PlayerCharacter>().Where(where, db));
		}
		...
		{
			var results = new PlayerCharacterCollection(Select<PlayerCharacterColumns>.From<PlayerCharacter>().Where(where, db));
		...
		public static PlayerCharacterCollection Top(int count, WhereDelegate<PlayerCharacterColumns> where, OrderBy<PlayerCharacterColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerCharacter>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerCharacterCollection>(0);
		}
