...
* Copyright 2013, Brevitee
* Available via the MIT or new BSD license.
*
* Utility functions plugged into underscore.js
*/
...
(function (b, d, $, dst, _) {
            ...
            dst.render(viewName, data, function(e, r){
                if(e){
                    ...
                    if (_.isString(opts) || _.isElement(opts)) { // assume its jquery selector or element
                        $(function(){
                            $(opts).html(r).data(viewName, data);
                        });
    ...
    function getInvokeConfig(args, urlFormat, format) {
        var strings = [],
            ...
            url = _.format(urlFormat, verb, format);
    ...
    b.invoke = function (className, method, args, format, options) {
        ...
        var root = b.proxyRoot(b[className].proxyName),
            urlFormat = root + "{0}/" + className + "/" + method + ".{1}?nocache=" + b.randomString(4) + "&",
        ...
        if(!crossDomain) {
            ...
            var url = _.format(urlFormat + "callback=?", "GET", "jsonp");
            config.url = url;
            setViewName(config);
