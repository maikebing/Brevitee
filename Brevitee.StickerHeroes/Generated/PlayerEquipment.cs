...
namespace Brevitee.StickerHeroes
{
	...
	public Player PlayerOfPlayerId
	{
		get
		{
			...
			{
				_playerOfPlayerId = Brevitee.StickerHeroes.Player.OneWhere(f => f.Id == this.PlayerId);
			}
	...
	public Equipment EquipmentOfEquipmentId
	{
		get
		{
			...
			{
				_equipmentOfEquipmentId = Brevitee.StickerHeroes.Equipment.OneWhere(f => f.Id == this.EquipmentId);
			}
		...
		{
			return new PlayerEquipmentCollection(new Query<PlayerEquipmentColumns, PlayerEquipment>(where, db), true);
		}
		...
		{
			return new PlayerEquipmentCollection(new Query<PlayerEquipmentColumns, PlayerEquipment>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerEquipmentCollection(Select<PlayerEquipmentColumns>.From<PlayerEquipment>().Where(where, db));
		}
		...
		{
			var results = new PlayerEquipmentCollection(Select<PlayerEquipmentColumns>.From<PlayerEquipment>().Where(where, db));
		...
		public static PlayerEquipmentCollection Top(int count, WhereDelegate<PlayerEquipmentColumns> where, OrderBy<PlayerEquipmentColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerEquipment>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerEquipmentCollection>(0);
		}
