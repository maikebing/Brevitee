...
namespace Brevitee.StickerHeroes
{
	...
	public Equipment EquipmentOfEquipmentId
	{
		...
		{
			return new PlayerOneEquipmentCollection(new Query<PlayerOneEquipmentColumns, PlayerOneEquipment>(where, db), true);
		}
		...
		{
			return new PlayerOneEquipmentCollection(new Query<PlayerOneEquipmentColumns, PlayerOneEquipment>(where, orderBy, db), true);
		}
		...
		{
			return new PlayerOneEquipmentCollection(Select<PlayerOneEquipmentColumns>.From<PlayerOneEquipment>().Where(where, db));
		}
		...
		{
			var results = new PlayerOneEquipmentCollection(Select<PlayerOneEquipmentColumns>.From<PlayerOneEquipment>().Where(where, db));
		...
		public static PlayerOneEquipmentCollection Top(int count, WhereDelegate<PlayerOneEquipmentColumns> where, OrderBy<PlayerOneEquipmentColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<PlayerOneEquipment>(): database;
			...
			query.Execute(db);
			return query.Results.As<PlayerOneEquipmentCollection>(0);
		}
