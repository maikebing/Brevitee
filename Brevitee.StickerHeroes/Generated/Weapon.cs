...
namespace Brevitee.StickerHeroes
{
	...
	public EffectOverTime EffectOverTimeOfEffectOverTimeId
	{
		get
		{
			...
			{
				_effectOverTimeOfEffectOverTimeId = Brevitee.StickerHeroes.EffectOverTime.OneWhere(f => f.Id == this.EffectOverTimeId);
			}
	...
	public RequiredLevelWeaponCollection RequiredLevelWeaponsByWeaponId
	{
		...
		{
			return new WeaponCollection(new Query<WeaponColumns, Weapon>(where, db), true);
		}
		...
		{
			return new WeaponCollection(new Query<WeaponColumns, Weapon>(where, orderBy, db), true);
		}
		...
		{
			return new WeaponCollection(Select<WeaponColumns>.From<Weapon>().Where(where, db));
		}
		...
		{
			var results = new WeaponCollection(Select<WeaponColumns>.From<Weapon>().Where(where, db));
		...
		public static WeaponCollection Top(int count, WhereDelegate<WeaponColumns> where, OrderBy<WeaponColumns> orderBy, Database database = null)
		{
			...
			Database db = database == null ? _.Db.For<Weapon>(): database;
			...
			query.Execute(db);
			return query.Results.As<WeaponCollection>(0);
		}
