...
namespace Brevitee.Messaging
{
    public abstract class EmailComposer: IEmailComposer
    {
        ...
        public static Email SetEmailSettings(Vault settingContainer, Email email)
        {
            ...
            Args.ThrowIf<ArgumentException>(!requiredSettingsSet, "Required settings missing from specified vault: {0}", string.Join(", ", requiredSettingMessages));
            Args.ThrowIf<ArgumentException>(!extendedSettingsSet, "Extended settings missing from specified vault: {0}", string.Join(", ", extendedSettingMessages));
