...
namespace Brevitee.BattleStickers.Business.Data
{
	...
	public PlayerTwoEquipmentCollection PlayerTwoEquipmentsByEquipmentId
	{
		...
		public static EquipmentCollection LoadAll(Database database = null)
		{
			...
			Database db = database == null ? Db.For<Equipment>(): database;
		...
		{
			var results = new EquipmentCollection(db, new Query<EquipmentColumns, Equipment>(where, db), true);
			return results;
		}
		...
		{
			var results = new EquipmentCollection(db, new Query<EquipmentColumns, Equipment>(where, orderBy, db), true);
			return results;
		}
		...
		{
			var results = new EquipmentCollection(db, Select<EquipmentColumns>.From<Equipment>().Where(where, db));
			return results;
		}
